...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        def SetExperimentValues(self,trial_number, wing_band, force, head_mass):
            user_trial = trial_number.get()
            user_wb = wing_band.get()
            user_f = force.get()
            user_mass = head_mass.get()
            self.experiment_params ={"Trial" : user_trial, "Wing Band" : user_wb, "Force" : user_f, "Mass" : user_mass}   
            return 0                   
